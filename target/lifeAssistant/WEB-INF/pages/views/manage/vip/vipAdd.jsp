...
<div id="main-wrap" class="animated fadeIn">
    ...
    <div class="main-content">
        <form class="layui-form" action="${ctx}/manage/vip/vipSave" accept-charset="utf-8" method="post">
            <input id="id" type="hidden" name="id" value="${vip.id}" required lay-verify="title" autocomplete="off" class="layui-input">
            <div class="layui-form-item">
                <label class="layui-form-label">昵称：</label>
                <div class="layui-input-block">
                    <input id="name" name="name" value="${vip.name}"  required  lay-verify="title" placeholder="如 小时光" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item layui-form-text">
                <label class="layui-form-label">地址：</label>
                <div class="layui-input-block">
                    <input id="address" name="address"  required value="${vip.address}" lay-verify="title" placeholder="如 南京市雨花台区XXX" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item layui-form-text">
                <label class="layui-form-label">手机号：</label>
                <div class="layui-input-block">
                    <input id="phone" name="phone"  required value="${vip.phone}" lay-verify="phone" placeholder="十一位" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
                <label class="layui-form-label">生日：</label>
                <div class="layui-input-block">
                    <input type="text" name="birthday" required id="birthday" lay-verify="date" value="<fmt:formatDate value="${vip.birthday}" type="date"/>" placeholder="yyyy-mm-dd" autocomplete="off" class="layui-input" onclick="layui.laydate({elem: this})">
                </div>
            </div>
            <div class="layui-form-item" style="margin-top: 20px">
                <div class="layui-input-block">
                    ...
                    <button type="button" class="layui-btn layui-btn-primary do-action" data-type="doGoBack" data-href="${ctx}/manage/vip/vipQuery">返回</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    layui.use('upload', function(){
        layui.upload({
            url: '${ctx}/manage/cate/uploadChildCateLogo'
            ,method: 'post'
            ...
            ,success: function(result){ //上传成功后的回调
                if(result.code == 0){
                    $('#fileName').attr("value", result.data.src);
                }else {
                    $('#fileName').attr("value", "上传失败");
                }
                layer.closeAll();
