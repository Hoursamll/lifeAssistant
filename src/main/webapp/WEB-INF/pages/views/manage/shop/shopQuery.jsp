...
<div id="main-wrap" class="animated fadeIn">
    <div class="main-title">
        <h1>商城活动列表</h1>
        <div class="toolbar">
            <button class="layui-btn layui-btn-small do-action" data-type="doAdd" data-href="${ctx}/manage/shop/shopAdd">
                <i class="layui-icon">&#xe608;</i> 添加商城活动
            </button>
        </div>
    </div>
    <hr>
        ...
        <table class="table-sort table-box dataTable">
            ...
            <c:forEach var="shop" items="${shop}">
                <tr>
                    ...
                    <td>${shop.number}</td>
                    <td>${shop.title}</td>
                    <td onclick="showParentPic('${shop.imgUrl}')">
                        <img style="width: 45px;" src="${shop.imgUrl}">
                    </td>
                    <td>${shop.message}</td>
                    <td><fmt:formatDate value="${shop.startDate}" type="date"/></td>
                    <td><fmt:formatDate value="${shop.endDate}" type="date"/></td>
                    <td>
                        <ul class="layui-nav" lay-filter="">
                            <li class="layui-nav-item">
                                <a href="javascript:;">操作</a>
                                <dl class="layui-nav-child">
                                    <dd><a href="javascript:;" class="do-action" data-type="doEdit" data-href="${ctx}/manage/shop/shopAdd?id=${shop.id}"><i class="layui-icon">&#xe642;</i>编辑</a></dd>
                                    <dd><a href="javascript:;" class="do-action" onclick="alertDel(${shop.id})"><i class="layui-icon">&#xe640;</i>删除</a></dd>
                                </dl>
                            </li>
                        </ul>
                    </td>
                ...
                ,yes: function(index, layero){
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            layer.closeAll();
                            window.location.href="${ctx}/manage/shop/shopQuery"
                        },
                        error: function (e) {
                            alert("删除失败！");
                            layer.closeAll();
